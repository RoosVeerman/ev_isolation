...
proteins_number <- matrix(0, nrow=8, ncol=length(proteins_eachmethod), dimnames = list(c("D1", "D2", "D3","batch 2", "batch 3", "batch 10", "batch 11", "Total identified proteins"),  names(proteins_eachmethod)))
...
#write.csv(proteins_number, "C:/Users/rosvee/Documents/11. Isolation project/proteomic_analysis/ev_isolation/output/script2/Protein_Counts.csv")
...
colourscheme <- (c("deepskyblue4", "deepskyblue3", "darkseagreen4", "darkseagreen3", "orangered4", "orangered2", "plum3", "plum 1", "goldenrod3", "goldenrod1"))

layout.matrix1 <- matrix(c(1,2,3,4), byrow= TRUE, nrow = 2, ncol = 2)
#pdf(file="C:/Users/rosvee/Documents/11. Isolation project/proteomic_analysis/ev_isolation/output/script2/PCA_sampletype_allproteins.pdf", width = 8, height = 6, compress = FALSE)
layout(layout.matrix1, heights = c(2.5,1), widths = c(1,1,1,1))
par(mar = c(4, 4, 2, 1))

plot(pc$x[,1:2], pch=20, col=colourscheme[c(7,1)][metadata[rownames(pc$x),"sample_type"]], las=1, cex=2.2, xlab=paste("PC1,", perc_pc[1], "%", sep=" "), ylab=paste("PC2,", perc_pc[2], "%", sep=" "))
#addTextLabels(pc$x[,1], pc$x[,2], labels = metadata$sample_name, cex.label = 0.5)
text(pc$x[,1] , pc$x[,2], labels = metadata[rownames(pc$x),"isolation_method"], pos = 3, cex=0.7)

plot(pc$x[,3:4], pch=20, col=colourscheme[c(7,1)][metadata[rownames(pc$x),"sample_type"]], las=1, cex=2.2, xlab=paste("PC3,", perc_pc[3], "%", sep=" "), ylab=paste("PC4,", perc_pc[4], "%", sep=" "))
#addTextLabels(pc$x[,1], pc$x[,2], labels = metadata$sample_name, cex.label = 0.5)
text(pc$x[,3] , pc$x[,4], labels = metadata[rownames(pc$x),"isolation_method"], pos = 3, cex=0.7)

mtext(paste("Top",length(topvarnames), sep=" "), outer=TRUE,  cex=1.5, line=-1.5)

par(mar = c(2, 4, 2, 1))
plot(0, type="n", axes=F, xlab="", ylab="")
legend("bottom", legend = levels(metadata[rownames(pc$x),"sample_type"]), fill=colourscheme[c(7,1)], cex = 0.7)
...
if (i == "plasma") {
colourscheme <- (c( "skyblue3", "skyblue4", "orangered2", "lightgoldenrod2", "darkseagreen3", "darkseagreen4", "plum 1", "plum3"))
}
else{
colourscheme <- (c("skyblue3", "orangered2", "lightgoldenrod2", "darkseagreen3",  "sienna1", "plum3"))
}

#pdf(file=paste0("C:/Users/rosvee/Documents/11. Isolation project/proteomic_analysis/ev_isolation/output/script2/PCA_donors_", i, "_sepnmethod.pdf"), width = 8, height = 6, compress = FALSE)
...
plot(j$x[,3:4], pch=20, col=colourscheme[][factor(metadata[rownames(j$x), "sample_complete"])], las=1, cex=2.6, xlab=paste("PC3,", perc_pc[3], "%", sep=" "), ylab=paste("PC4,", perc_pc[4], "%", sep=" "))
text(j$x[,3] , j$x[,4], labels = metadata[rownames(j$x), "donor_code"], pos = 3, cex=0.7)
...
#make matrix with a 1 or 0 value for every detected protein/method
table_upset<- matrix(0, nrow =length(allproteins), ncol = length(proteins_eachmethod), dimnames = list(allproteins,names(proteins_eachmethod)))

#fill in table
for (i in colnames(table_upset)){
  tmplist <- proteins_eachmethod[[i]]
  tmplog <- row.names(table_upset) %in% tmplist
  table_upset[,i] <- tmplog
}

#make sets for upset plot, show only when more than 1 protein is shared between groups

upsetplot_plasma_table <- as.data.frame.array(table_upset[,1:8])

upsetplot_mm6_table <- as.data.frame.array(table_upset[,9:14])

upsetplot_plasma <- upset(upsetplot_plasma_table, nsets = 8, 
    sets.bar.color = "#56B4E9", point.size = 3, line.size = 1.3, 
    mb.ratio = c(0.70, 0.30), empty.intersections = TRUE,  
    set_size.show = TRUE, scale.sets = "identity")

upsetplot_mm6 <- upset(upsetplot_mm6_table, nsets = 8,
   sets.bar.color = "#56B4E9", point.size = 3, line.size = 1.3,     
   empty.intersections = TRUE,  set_size.show = TRUE, 
   scale.sets =    "identity")


#pdf(file=paste0("C:/Users/rosvee/Documents/11. Isolation project/proteomic_analysis/ev_isolation/output/script2/upset_plasma.pdf"), width = 20, height = 6, compress = FALSE)
...
#pdf(file=paste0("C:/Users/rosvee/Documents/11. Isolation project/proteomic_analysis/ev_isolation/output/script2/upset_mm6.pdf"), width = 26, height = 6, compress = FALSE)
